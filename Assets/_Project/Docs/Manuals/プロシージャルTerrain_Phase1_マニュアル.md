...
> Inspector から `terrainGuid` に任意の文字列（例: `tile_00_00`）を入力し、
> `SetPaths()` を呼ぶか、直接パスフィールドを記入してください。
...
2. `TerrainPersistentData` を作成し、Inspector で `terrainGuid` を入力後 `SetPaths()` を実行する
3. シーンに Terrain を配置し、Terrain フィールドに設定する
4. **Generate** を押す
