...
* Prefab のローカル `+Y` を **ワールド上方向** に一致させる
* v0.1 では水平 Roof 面のみ対象とするため、特殊な傾斜回転は不要
...
* v0.1 の `FlatRoof` は `allowScale = true` を持つ屋根モジュールとして扱う
...
* 壁幅の出所を固定値から基準壁モジュール参照へ変更
* 屋根 Pivot 規約を追加
* Rebuild All 時の Registry 更新手順を明記
* 再マッピング閾値と複合スコア式を数値で固定
* 横方向グリッド起点を明記
* `baseElevation` の用途を定義
* `nominalSize` と `allowScale` の役割分離を維持
* `GeneratedObjects` と `GeneratedObjectRegistry` の責務分離を維持

現時点で v0.1 実装に必要な主要仕様は一通り明文化できているが、
実装着手後に細部の API 設計やクラス責務の調整が入る可能性はある。
