...
| `MaskType` | マスク種別列挙体（Protected / NoVegetation / Flatten） |
| `MaskEditor` | マスク配列への数値範囲・ブラシ編集適用（Phase 3A / 3B 共通） |
| `MaskOverlayRenderer` | マスクデータを Terrain XZ 平面に半透明テクスチャとして投影するエディタユーティリティ |
...
- [ ] マスクオーバーレイの HDRP 対応（Issue: MaskOverlay_HDRP_issue.md）
- [ ] Phase 4A: テクスチャ塗りの実装（Issue: Phase4_issue.md）
- [ ] Phase 4B: 木・草配置の実装（Issue: Phase4_issue.md）
...
| 2026-04-03 | Phase 3A/3B 実装（MaskType / MaskEditor・Mask タブ UI・protectedMask の Generate 統合） |
| 2026-04-03 | マスクオーバーレイ実装（MaskOverlayRenderer・SceneView への半透明テクスチャ投影） |
| 2026-04-03 | MaskOverlayRenderer を GL 方式に変更（HDRP SceneView 対応・Hidden/Internal-Colored 使用） |
| 2026-04-03 | Mask / Vegetation タブを凍結・封印。Issue 起票（MaskOverlay_HDRP_issue / Phase4_issue） |
